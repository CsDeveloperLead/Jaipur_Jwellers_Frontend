                    ...
                    products.length > 0
                        ? products.map((product, index) => (
                            <NavLink state={{ product }} to={`/single-product/${product._id}`} key={index} onMouseEnter={() => setHoverNo(index)} onMouseLeave={() => setHoverNo('')} className={`${hoverNo === index ? 'md:h-[340px] xl:h-[450px]' : 'md:h-[310px] xl:h-[400px]'} min-w-[70vw] h-auto pb-4 bg-gray-200 flex flex-col rounded-3xl p-1 sm:min-w-[40vw] md:min-w-[30vw] lg:min-w-[25vw] `}>
                                <img src={selectedImages[product._id] || product?.Image} alt="image" className='w-full h-60 bg-gray-900 rounded-3xl xl:h-[470px]' />
                                ...
                                <div className={`${hoverNo === index ? 'hidden md:w-full md:h-[30px] xl:h-[50px] md:flex md:justify-between md:mt-4 md:items-center md:px-2' : 'hidden'} `} >
                                    <span className='flex gap-2'>
                                        <span
                                            ...
                                            className="h-4 w-4 rounded-full xl:h-6 xl:w-6 border-[1px] cursor-pointer" style={{ backgroundColor: `#${product?.Image1.color}` }}></span>
                                        <span
                                            onClick={(e) => {
                                                e.preventDefault()
                                                e.stopPropagation();
                                            ...
                                            className="h-4 w-4 rounded-full xl:h-6 xl:w-6 border-[1px] cursor-pointer" style={{ backgroundColor: `#${product?.Image2.color}` }}></span>
                                        <span
                                            onClick={(e) => {
                                                e.preventDefault()
                                                e.stopPropagation();
                                            ...
                                            className="h-4 w-4 rounded-full xl:h-6 xl:w-6 border-[1px] cursor-pointer" style={{ backgroundColor: `#${product?.Image3.color}` }}></span>
                                    </span>
